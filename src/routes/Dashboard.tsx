...
export function Dashboard({ patients }: { patients: Patient[] }) {
  ...
  const referral = Object.entries(
    patients.reduce<Record<string, number>>((a, p) => ((a[p.recommendedSpecialist] = (a[p.recommendedSpecialist] || 0) + 1), a), {}),
  ).map(([name, value]) => ({ name, value }))
  const sorted = [...patients].sort((a, b) => b.aiRiskScore - a.aiRiskScore)

  const insightData = [
    { title: 'SLA Review', value: `${Math.max(84, 100 - Math.round((review / Math.max(patients.length, 1)) * 100))}%`, tone: 'text-emerald-700 bg-emerald-50' },
    { title: 'Akurasi AI', value: '96.2%', tone: 'text-sky-700 bg-sky-50' },
    { title: 'Kepatuhan Follow-up', value: '89%', tone: 'text-amber-700 bg-amber-50' },
  ]
    ...
    <div className="space-y-5">
      ...
      <div className="grid gap-3 md:grid-cols-3">
        {insightData.map((insight) => (
          <div key={insight.title} className="card flex items-center justify-between p-4">
            <p className="text-sm text-slate-500">{insight.title}</p>
            <span className={`rounded-full px-3 py-1 text-sm font-semibold ${insight.tone}`}>{insight.value}</span>
          </div>
        ))}
      </div>
      ...
      <RiskSummaryCards
        stats={{
          ...
          'LASIK eligible': lasik,
          'Diabetes retina screening': diabeticScreen,
          'Post-op red flag': postOp,
        }}
      />
      <div className="grid gap-5 xl:grid-cols-2">
        <AIPriorityQueue patients={sorted} />
        <BranchPerformanceChart data={mockBranches} />
      </div>
      <div className="grid gap-5 xl:grid-cols-2">
        <ReferralDistributionChart data={referral} />
        <PatientRiskTable patients={sorted} />
      </div>
    </div>
  )
}
